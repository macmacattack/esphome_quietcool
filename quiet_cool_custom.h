...
class QuietCoolTransmitter : public Component, public spi::SPIDevice<spi::BIT_ORDER_MSB_FIRST, spi::CLOCK_POLARITY_LOW, spi::CLOCK_PHASE_LEADING, spi::DATA_RATE_2MHZ> {
 public:
  gpio_num_t cs_pin   = GPIO_NUM_10; // ESP32-S3 GPIO10
  gpio_num_t gdo0_pin = GPIO_NUM_14; // ESP32-S3 GPIO14
  ...
  void init_cc1101() {
    ...
    // Read CC1101 Version Register to verify hardware connection (0x31 | 0xC0 = 0xF1)
    uint8_t ver = this->read_reg(0xF1);
    ESP_LOGI("quiet_cool", "CC1101 Hardware Version Read: 0x%02X", ver);

    if (ver == 0x00 || ver == 0xFF) {
      ESP_LOGE("quiet_cool", "SPI BUS FAILURE! Check GPIO10(CS), GPIO11(MOSI), GPIO12(CLK), GPIO13(MISO) wiring!");
      return;
    }

    // CC1101 Register Setup for QuietCool RF
    this->write_reg(0x00, 0x29); 
    this->write_reg(0x01, 0x2E); 
    this->write_reg(0x02, 0x2D); // GDO0 Direct Async Transmission
    ...
    this->write_reg(0x3E, 0xC0); // Maximum PA TX Power Output (+10 dBm)

    this->write_strobe(0x36); // SIDLE
    ESP_LOGI("quiet_cool", "CC1101 ESP32-S3 Hardware Ready.");
  }

  // Microsecond bit transmission inside a critical spinlock
  void send_bits_from_bytes(const uint8_t *data, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
      ...
      for (int bit = 7; bit >= 0; bit--) {
        ...
        ets_delay_us(400); // Strict 400us URH pulse
      }
    }
  ...
  void transmit_command(uint8_t speed, uint8_t duration) {
    ...
    ESP_LOGD("quiet_cool", "Broadcasting CMD 0x%02X to QuietCool Fan...", cmd_byte);

    for (int i = 0; i < 8; i++) {
      gpio_set_level(this->gdo0_pin, 0);
      this->write_strobe(0x35); // STX (Start Transmit Mode)
      
      ets_delay_us(1000); 
      
      this->send_bits_from_bytes(packet, 20);

      this->write_strobe(0x36); // SIDLE (Return to Idle)
      
      App.feed_wdt();
      vTaskDelay(pdMS_TO_TICKS(20)); // Feed FreeRTOS watchdog between repeats
    }
  }
