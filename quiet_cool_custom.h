...
class QuietCoolTransmitter : public Component, public spi::SPIDevice<spi::BIT_ORDER_MSB_FIRST, spi::CLOCK_POLARITY_LOW, spi::CLOCK_PHASE_LEADING, spi::DATA_RATE_2MHZ> {
 public:
  ...
  // Remote ID matching paired unit
  ...
  void init_cc1101() {
    ...
    // CC1101 direct mode register setup
    ...
    // 2500 Baud Rate Calibration (400us per symbol)
    ...
    ESP_LOGI("quiet_cool", "CC1101 ESP32-S3 Precision Driver Ready.");
  }

  // Hardware spinlock critical section to guarantee exact 400us timing
  void send_bits_from_bytes(const uint8_t *data, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
      ...
      for (int bit = 7; bit >= 0; bit--) {
        ...
        ets_delay_us(400); // Strict 400us pulse
      }
    }
  ...
  void transmit_command(uint8_t speed, uint8_t duration) {
    ...
    ESP_LOGD("quiet_cool", "Transmitting CMD 0x%02X (8 repeats)...", cmd_byte);
    ...
    for (int i = 0; i < 8; i++) {
      ...
      App.feed_wdt();
      vTaskDelay(pdMS_TO_TICKS(20)); // Yield to FreeRTOS between bursts
    }
  }
