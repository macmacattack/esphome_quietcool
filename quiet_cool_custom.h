...
class QuietCoolSniffer : public Component, public spi::SPIDevice<spi::BIT_ORDER_MSB_FIRST, spi::CLOCK_POLARITY_LOW, spi::CLOCK_PHASE_LEADING, spi::DATA_RATE_2MHZ> {
 public:
  ...
  void init_cc1101() {
    ...
    // --- CONFIGURE RADIO FOR RAW RECEIVE MODE ---
    this->write_reg(0x00, 0x0D); // IOCFG2: CRITICAL - Outputs raw received data to GDO2!
    this->write_reg(0x01, 0x2E); 
    this->write_reg(0x02, 0x2D); // IOCFG0: High impedance
    ...
    this->write_reg(0x08, 0x32); // PKTCTRL0: Async Serial Mode
    
    // Exact same frequency and baud settings as before
    ...
    // Start Listening!
    this->write_strobe(0x34); // SRX (Enable Receive)
    ESP_LOGI("quiet_cool", "============================================================");
    ESP_LOGI("quiet_cool", "CC1101 IS NOW IN SNIFFER MODE - LISTENING ON GDO2 (Pin 4)");
    ESP_LOGI("quiet_cool", "============================================================");
  }
};
